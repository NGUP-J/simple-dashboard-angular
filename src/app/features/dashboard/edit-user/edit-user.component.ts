...
export class EditUserComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
        ...
        if (this.id) {
          this.paramsSubscription =  this.userService.getUserById(this.id).subscribe({
            next: (response) => {
              this.user = response;
              this.edit = {
  ...
  onFormSubmit(): void {
    ...
    // this.resetForm();
    if (this.id) {
      this.editUserSubscription = this.userService.editUser(this.id, EditUserRequest).subscribe({
        next: (response) => {
          console.log('User edited successfully');
          this.router.navigate(['/dashboard']);
        },
